...
function CashBook() {
  ...
  useEffect(() => {
    const { accessToken, cashierId, branchId } = getAuthUser();
    API.defaults.headers.common.Authorization = `Bearer ${accessToken}`;

    API.get(`/cashiers/cash-book/${cashierId}/${branchId}`)
      .then((result) => {
      ...
      {cashBook && (
        <div className="flex flex-col gap-4 rounded-lg border border-gray-200 bg-white p-8 text-sm text-gray-600 md:min-w-[550px] md:p-16 md:text-xl">
          <div className="flex w-full flex-row items-center justify-between">
            <h2 className="w-2/3 whitespace-nowrap font-bold">Last Checkout Date:</h2>

            <span className="w-1/3 whitespace-nowrap text-green-600">
              {cashBook.lastCheckOutDate && format(new Date(cashBook.lastCheckOutDate), "yyyy-MM-dd")} G.C
            </span>
          </div>
          ...
          <div className="flex w-full flex-row justify-between">
            <h2 className="w-2/3 whitespace-nowrap font-bold ">Due Cash(in Birr):</h2>

            <span className="w-1/3 whitespace-nowrap text-green-600">{cashBook.dueCash}</span>
          </div>
        </div>
      )}
    </div>
